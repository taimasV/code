...
export function scoreDart(x: number, y: number): DartThrow {
  ...
  if (radius > 0.84) return { label: 'Miss', score: 0, x, y };
  if (radius <= 0.05) return { label: 'Bullseye', score: 50, x, y };
  if (radius <= 0.11) return { label: 'Outer bull', score: 25, x, y };

  const angle = (Math.atan2(dx, -dy) + Math.PI * 2) % (Math.PI * 2);
  const sector = SECTORS[Math.floor((angle + Math.PI / 20) / (Math.PI / 10)) % 20];
  const multiplier = radius >= 0.76 ? 2 : radius >= 0.46 && radius <= 0.54 ? 3 : 1;
